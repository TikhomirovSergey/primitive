...
public class Log {
	...
	private static final ILogConverter converting = (ILogConverter) Proxy.newProxyInstance(
			ILogConverter.class.getClassLoader(),
	...
	private static Level resetLogLevel()
	{
		...
		if (level==null)
		{	//if there is no customized setting for log level
			commonLevel = info;
		}
		else
		{
	...
	private static LogRecWithAttach getRecordForLog(Level level, String msg)
	{
		StackTraceElement stack[] = (new Throwable()).getStackTrace();
		StackTraceElement element = stack[levelUp];
		LogRecWithAttach rec = new LogRecWithAttach(level, msg);
	...
	public static void log(Level level, String msg, File attached)
	{
		applyLogRec(getRecordForLog(level, msg), attached);
	}
	
	public static void log(Level level, String msg, Throwable t, File attached)
	{
	...
	{
		applyLogRec(setThrown(getRecordForLog(eAvailableLevels.FINE, msg),t), attached);
	}		
	...
	{
		applyLogRec(setThrown(getRecordForLog(eAvailableLevels.INFO, msg),t), attached);
	}	
	...
	{
		applyLogRec(setThrown(getRecordForLog(eAvailableLevels.WARN, msg),t), attached);
	}	
	...
	{
		applyLogRec(setThrown(getRecordForLog(eAvailableLevels.SEVERE, msg),t),attached);
	}
