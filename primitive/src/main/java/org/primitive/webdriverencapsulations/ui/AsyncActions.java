...
/**
 * @author s.tihomirov org.openqa.selenium.interactions.Actions with asynchron
 *         execution
 */
public class AsyncActions extends Actions {
	/**
	 * @author s.tihomirov Performs interactive actions in another thread
	 *         asynchronously
	 */
	private class AsyncPerformer extends Thread {
		private Actions action;
		private boolean isExecuting = false; 
		// Are actions executed yet? If time has passed.
		private RuntimeException error; // it will refer to some exception if it
		// is caught
		...
		private void execute(long secsToWait) throws RuntimeException {
			...
			while ((isExecuting)
					& (Math.abs(currentTime - startTime) < secsToWait * 1000)) {
			...
			if (isExecuting) {
				Log.message("Actions have not been completed. Specified time: "
						+ Long.toString(secsToWait) + " sec. "
		...
		public void run() {
			try {
				...
				Log.warning(
						"During execution of specified actions the problem has been found out: "
								+ e.getMessage(), e);
				error = e;
			}
			isExecuting = false;
